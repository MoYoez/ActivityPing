...
- **macOS**: foreground app capture uses the native bridge, but window title capture requires Accessibility permission and media capture uses a bundled `mediaremote-adapter` (prepared automatically during `pnpm tauri dev` / `pnpm tauri build`).
