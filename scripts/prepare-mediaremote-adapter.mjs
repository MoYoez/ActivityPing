...
function run(program, args, options = {}) {
  ...
  if (result.error) {
    throw new Error(`${program} failed: ${result.error.message}`);
  }
...
function ensureSourceCheckout() {
  ...
  run("git", ["clone", "--depth", "1", "--branch", REPO_TAG, REPO_URL, sourceRoot]);
  ...
  if (currentHead !== REPO_COMMIT) {
    throw new Error(
      `mediaremote-adapter version mismatch: got ${currentHead}, expected ${REPO_COMMIT}`,
    );
  }
}
